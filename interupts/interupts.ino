...
static inline int SinSample(double pos) {
  ...
  /*if(sample_floor == WAVEFORM_SAMPLES-1)
    return waveformsTable[0][sample_floor];
  else
    return waveformsTable[0][sample_floor]*(1-frac) + waveformsTable[0][1+sample_floor]*frac;*/
  //Serial.println(frac);

  //return (int)((1-frac)*waveformsTable[0][sample_floor] + frac*waveformsTable[0][sample_floor]);
  /*int val = (int)(0.5*waveformsTable[0][sample_floor]+0.5*waveformsTable[0][sample_floor]);
  if(val-waveformsTable[0][sample_floor] > 0.5) {
    Serial.println("test");
    return 0;
  }*/
  //Serial.println(val-waveformsTable[0][sample_floor]);
...
static inline int nextSample() {
  ...
  int sample = sampleWaveform(sintest, 512, sample_position / sample_period);
