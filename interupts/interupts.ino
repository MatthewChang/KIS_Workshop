...
void loop()
{
  ...
  average_window[window_position] = in;
  window_position = (window_position + 1)%window_size;
  float expected_position = 0;
  for(int i = 0; i < window_size; i++) {
    expected_position += average_window[i];
  }
  expected_position /= window_size;
  
  Serial.println(expected_position);
  //frequency = frequencyFromVoltage(in,BLUES_C,BLUES_C_SIZE);
  frequency = frequencyFromRange(expected_position,0,30,BLUES_C_SMALL,BLUES_C_SMALL_SIZE);
